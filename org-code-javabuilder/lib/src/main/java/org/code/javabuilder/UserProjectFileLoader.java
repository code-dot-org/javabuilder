...
public class UserProjectFileLoader implements ProjectFileLoader {
   ...
   *
   * @throws InternalError If the load times out or fails.
   */
  @Override
  ...
  private String getRequest(String url, HttpClient client) throws InternalError {
    HttpRequest request =
    ...
    if (response.statusCode() > 299) {
      throw new InternalError(InternalErrorKey.INTERNAL_EXCEPTION, new Exception(body));
    }
    return body;
  }
}
